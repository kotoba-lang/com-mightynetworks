(ns mightynetworks.main
  "Kotodama WASM entrypoint for the Mightynetworks clean-room actor (L4) — Clojure port.
  ...
  facts (`mightynetworks.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "mightynetworks")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"mightynetworks.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mightynetworks-compat" :tier tier :entities entities} 200])
...
(defn start! [] :mightynetworks-compat/ready)
